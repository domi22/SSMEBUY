...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
			<div class="search">
				<form method="post" action="indent/searchpage?usecurrentPage=-1">
					订单号：<input type="text" class="text" name="indentid" /> 订货人：<input type="text" class="text" name="username" /> <label class="ui-blue"><input type="submit" name="submit" value="查询" /></label>
				</form>
			</div>
			...
			<table class="list">
				...
				<c:forEach items="${requestScope.indetlist}" var="indent">
				   <c:forEach items="${indent.userlist}" var="user">
				        <c:forEach items="${indent.statuslist}" var="status">
				             <tr>
								<td class="first w4 c">${indent.indentid}</td>
								<td class="w1 c">${user.username}</td>
								<td class="w1 c">${user.address}</td>
								<td class="w1 c">${user.phone}</td>
								<td class="w1 c">${status.statusname}</td>
								<td class="w1 c"><a href="indent/tomdf?indentid=${indent.indentid}&username=${user.username}&address=${user.address}&phone=${user.phone}&statusname=${status.statusname}&usecurrentPage=${requestScope.usecurrentPage}">修改</a> <a onclick="confirm('你确定要删除?')?location.href='indent/del?indentid=${indent.indentid}&usecurrentPage=${requestScope.usecurrentPage}':location.href='indent/page?usecurrentPage=${requestScope.usecurrentPage}'">删除</a></td>
						     </tr>
				        </c:forEach>
				   </c:forEach>
				</c:forEach>
			...
			<div class="pager">
			<ul class="clearfix">
					<li><a href="indent/page?usecurrentPage=1">首页</a></li>
					<c:choose>
				        <c:when test="${requestScope.usecurrentPage==1}">
				            <li><a href="javascript:void(0)" style="color:gray">上一页</a></li>
				        </c:when>
				        <c:otherwise>
				            <li><a href="indent/page?usecurrentPage=${requestScope.usecurrentPage-1}">上一页</a></li>
				        </c:otherwise>
				    </c:choose>
					<li class="current">第${requestScope.usecurrentPage}页</li>
					...
					<c:choose>
				        ...
				        <c:otherwise>
				           <li><a href="indent/page?usecurrentPage=${requestScope.usecurrentPage+1}">下一页</a></li>
				        </c:otherwise>
				    </c:choose>
				    <c:choose>
				        <c:when test="${requestScope.totalPage==0}">
				           <li><a href="indent/page?usecurrentPage=1">末页</a></li>
				        </c:when>
				        <c:otherwise>
				           <li><a href="indent/page?usecurrentPage=${requestScope.totalPage}">末页</a></li>
				        </c:otherwise>
				    </c:choose>
				</ul>
			</div>
