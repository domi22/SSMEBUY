	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<c:forEach items="${requestScope.newslist}" var="onenew">
				    <tr>
						<td class="first w4 c">${onenew.newid}</td>
						<td><a href="news/view?newid=${onenew.newid}" target="_blank">${onenew.newtitle}</a></td>
						<td class="w1 c"><a href="news/mdf?newid=${onenew.newid}&currentPage=${requestScope.currentPage}">修改</a> <a onclick="confirm('你确定要删除?')?location.href='news/del?newid=${onenew.newid}&currentPage=${requestScope.currentPage}':location.href='news/page?currentPage=${requestScope.currentPage}'">删除</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
		...
		<div class="pager">
			<ul class="clearfix">
					<li><a href="news/page?currentPage=1">首页</a></li>
					<c:choose>
				        <c:when test="${requestScope.currentPage==1}">
				            <li><a href="javascript:void(0)" style="color:gray">上一页</a></li>
				        </c:when>
				        <c:otherwise>
				            <li><a href="news/page?currentPage=${requestScope.currentPage-1}">上一页</a></li>
				        </c:otherwise>
				    </c:choose>
					...
					<c:choose>
				        ...
				        <c:otherwise>
				           <li><a href="news/page?currentPage=${requestScope.currentPage+1}">下一页</a></li>
				        </c:otherwise>
				    </c:choose>
				    <c:choose>
				        <c:when test="${requestScope.totalPage==0}">
				           <li><a href="news/page?currentPage=1">末页</a></li>
				        </c:when>
				        <c:otherwise>
				           <li><a href="news/page?currentPage=${requestScope.totalPage}">末页</a></li>
				        </c:otherwise>
				    </c:choose>
				</ul>
			</div>
