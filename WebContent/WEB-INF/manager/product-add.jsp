...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>添加商品</h2>
		<div class="manage">
			<form action="shangp/addshang" method="post" enctype="multipart/form-data">
				<table class="form">
					<tr>
						<td class="field">商品名称：</td>
						<td><input type="text" class="text" name="shangname" value="" /></td>
					</tr>
					<tr>
							...
							<select name="goodid">
							  <c:forEach items="${sessionScope.leftList}" var="fathergood">
			                        <c:if test="${fathergood.fatherid==0}">
						                 <option value="${fathergood.goodid}">${fathergood.goodname}</option>
						            <c:forEach items="${sessionScope.leftList}" var="songood">
						              <c:if test="${fathergood.goodid==songood.fatherid}">
						                <option value="${songood.goodid}">├  ${songood.goodname}</option>
						              </c:if>
						            </c:forEach>
						        </c:if>
						      </c:forEach>
					...
					<tr>
						...
						<td class="field"id="hderrorinfo" style="margin-left:0px">${requestScope.uploadspError}</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="text" class="text" name="shangdic" /></td>
					</tr>
					<tr>
						<td class="field">库存：</td>
						<td><input type="text" class="text tiny" name="restcount" /></td>
					</tr>
